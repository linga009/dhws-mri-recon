#!/usr/bin/env python3
"""dhws_mri_efficient.py - DHWS-MRI-Efficient: Hallucination-Free MRI Reconstruction

Key Features:
  1. Hallucination-Free: No GAN. Strict Hard Data Consistency (DC) projection.
  2. A100-scaled architecture: embed_dim=1024, K=256 spectral bases, 10-step cascade.
  3. Multi-Coil Native: SENSE SensitivityModel included.
  4. Mathematically Sound: float16 AMP, DC scaling, Fourier-pair normalization.
  5. Auto-resumes from checkpoint.pt if Colab disconnects.
"""
...
# A100 Tensor Core Optimization: Enable TF32 for massive float32 speedups
...
class MRIConfig:
    ...
    enc_base:       int   = 64         # Encoder base width
    embed_dim:      int   = 1024       # Scaled up for A100 ("No-Limits" config)
    n_spec_bases:   int   = 256        # Ultimate dictionary capacity
    hash_levels:    int   = 14         # Deepened for extreme texture resolution
    hash_dim:       int   = 8          # Massive texture vocabulary
    hash_out_ch:    int   = 48         # Wider feature map for refiner
    hash_res:       int   = 64
    refine_base:    int   = 64         # Wide U-Net for hard DC projection clean-up
    n_cascade:      int   = 10         # Ultimate unrolling depth
    num_coils:      int   = 1

    batch_size:     int   = 8          # Safely fits the 10-step unrolled graph in 40GB A100 VRAM
    epochs:         int   = 150
    warmup_epochs:  int   = 5
    lr:             float = 3e-4       # Scaled down slightly to match the batch_size of 8
...
def _hf_snapshot_to_dataroot(data_root: Path) -> bool:
    """
    Downloads AUMLProject/fastmri-knee-singlecoil-rss via snapshot_download
    and copies all .npz files into data_root.
    Returns True if at least one file is available afterwards.

    Background: load_dataset() with trust_remote_code no longer works for this
    repo (the HF datasets library dropped that flag).  snapshot_download grabs
    the raw repository files directly, which are the actual .npz volumes.
    File layout confirmed: each .npz has key 'images' of shape (N_slices, 640, 372).
    """
    try:
        from huggingface_hub import snapshot_download
        import shutil
    except ImportError:
        print("[!] pip install huggingface_hub  — needed for auto-download")
        return False

    REPO_ID = "AUMLProject/fastmri-knee-singlecoil-rss"
    print(f"[*] Downloading '{REPO_ID}' via snapshot_download (first run: several minutes)...")
    try:
        cache_dir = Path(snapshot_download(repo_id=REPO_ID, repo_type="dataset"))
        data_root.mkdir(parents=True, exist_ok=True)
        # HF stores data files under a 'data/' subfolder; fall back to root
        search_dir = cache_dir / "data" if (cache_dir / "data").exists() else cache_dir
        copied = 0
        for f in search_dir.iterdir():
            if f.suffix == ".npz":
                dest = data_root / f.name
                if not dest.exists():
                    shutil.copy(f, dest)
                copied += 1
        if copied:
            print(f"[*] {copied} .npz files ready in {data_root}")
            return True
        print(f"[!] snapshot_download succeeded but no .npz files found in {cache_dir}")
...
def get_loaders(cfg: MRIConfig):
    ...
    Priority:
      1. Local .npz files at cfg.data_root  (put them there with the Colab
         snapshot_download cell, or manually copy from Drive)
      2. Auto-download via snapshot_download → save to cfg.data_root
      3. Synthetic fallback (for smoke-testing only)

    Each .npz in this dataset has key 'images' of shape (N, 640, 372);
    FastMRINPZDataset resizes slices to cfg.image_size and synthesises
    undersampled k-space via rfft2 + Cartesian mask.
    """
    from torch.utils.data import random_split
    kw = dict(pin_memory=cfg.amp, persistent_workers=False)

    def _make_loaders(root):
        full_ds = FastMRINPZDataset(root, cfg)
        n_val   = max(1, int(0.2 * len(full_ds)))
        train_ds, val_ds = random_split(
            full_ds, [len(full_ds) - n_val, n_val],
            generator=torch.Generator().manual_seed(42)
        )
        tl = DataLoader(train_ds, cfg.batch_size, shuffle=True,  num_workers=8, **kw)
        vl = DataLoader(val_ds,   cfg.batch_size, shuffle=False, num_workers=8, **kw)
        return tl, vl
    ...
    if data_root.exists() and any(data_root.rglob("*.npz")):
        ...
        return _make_loaders(data_root)

    # 2. snapshot_download
    if _hf_snapshot_to_dataroot(data_root):
        return _make_loaders(data_root)

    # 3. Synthetic fallback
    print("[!] No real data found — using synthetic fallback")
    tl = DataLoader(SyntheticMRIDataset(1000, cfg), cfg.batch_size, shuffle=True)
    vl = DataLoader(SyntheticMRIDataset(200,  cfg), cfg.batch_size, shuffle=False)
    return tl, vl
...
class FastMRINPZDataset(Dataset):
    ...
    def _n_slices(self, d) -> int:
        for key in ('target', 'reconstruction_rss', 'reconstruction_esc',
                    'kspace', 'kspace_real', 'input', 'images'):
            if key in d.files:
                arr = d[key]
    ...
    def _load_slice(self, path: Path, s: int):
        ...
        if 'kspace_real' in d.files and 'kspace_imag' in d.files:
        ...
        elif 'input' in d.files or 'images' in d.files:
            # Layout C/F — image array (zero-filled or raw), synthesise undersampled k-space
            key = 'input' if 'input' in d.files else 'images'
            inp = d[key].astype(np.float32)
            ...
            return k2ch, mask.float(), torch.from_numpy(target[np.newaxis]).float()
        ...
        # Normalize BOTH by the exact same scale to preserve Fourier pair
        mx = target.max()
        if mx > 0:
            target = target / mx
            kc = kc / mx
        ...
        return k2ch, mask, torch.from_numpy(target[np.newaxis]).float()
...
class KSpaceEncoder(nn.Module):
    def __init__(self, embed_dim: int = 256, base: int = 32, image_size: int = 320):
        ...
        # Use a full conv block for projection to learn better features
        self.mag_proj = nn.Sequential(nn.Conv2d(1, base // 2, 3, padding=1), nn.GELU())
        self.phase_proj = nn.Sequential(nn.Conv2d(1, base // 2, 3, padding=1), nn.GELU())
...
class SpectralBasisDecoder(nn.Module):
    def __init__(self, image_size, channels, embed_dim, n_bases, enc_ch1=32, enc_ch2=64):
        ...
        self.pre_film = nn.Conv2d(enc_ch2, channels * 2, 1)
...
class ConcatFuse(nn.Module):
    def __init__(self, channels=1, hash_ch=48):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(64, channels, 1), nn.Sigmoid()
        )
...
class CascadeRefiner(nn.Module):
    def __init__(self, channels=1, hash_ch=48, base=64, enc_ch1=64, enc_ch2=128):
        super().__init__()
        ...
        # Zero-init: at epoch 0 the network is a pure DC projector (residual = 0).
        # This prevents the 10× cascade compounding of random-weight residuals that
        # causes loss=inf on the first epoch.
...
class DHWSMRIEfficient(nn.Module):
    def __init__(self, cfg: MRIConfig):
        ...
        enc_base = cfg.enc_base

        self.sens_model = SensitivityModel() if self.multicoil else None
        self.encoder = KSpaceEncoder(cfg.embed_dim, base=enc_base, image_size=cfg.image_size)
        self.spectral = SpectralBasisDecoder(cfg.image_size, cfg.channels, cfg.embed_dim, cfg.n_spec_bases, enc_ch1=enc_base, enc_ch2=enc_base*2)
        self.hashfeat = HashFeatureExtractor(cfg.embed_dim, cfg.image_size, cfg.hash_levels, cfg.hash_dim, hidden=64, hash_res=cfg.hash_res, out_ch=cfg.hash_out_ch)
        self.fusion = ConcatFuse(cfg.channels, hash_ch=cfg.hash_out_ch)
        # Weight-shared unrolled cascade — one set of weights, n_cascade forward passes
        shared_refiner = CascadeRefiner(cfg.channels, hash_ch=cfg.hash_out_ch, base=cfg.refine_base, enc_ch1=enc_base, enc_ch2=enc_base*2)
        self.cascade = nn.ModuleList([shared_refiner] * cfg.n_cascade)
    ...
    def forward(self, kspace, mask):
        if self.multicoil:
            ...
            # This is the zero-filled multi-coil image reconstruction
            combined_img = sense_reduce(kmc, sens) # (B, H, W) complex image
            # To get the equivalent single-coil k-space, we must FFT this image
            k_singlecoil_equiv = torch.fft.rfft2(combined_img, norm="ortho") # (B, H, fw) complex
            k2ch = torch.stack([k_singlecoil_equiv.real, k_singlecoil_equiv.imag], dim=1) # (B, 2, H, fw)
        else:
            k2ch = kspace
            kmc = torch.complex(kspace[:,0], kspace[:,1])
            sens = None
...
def train_epoch(model, optimizer, scheduler, loader, criterion, cfg, scaler):
    import warnings
    # torch.compile's ordering checker mis-fires on OneCycleLR + GradScaler;
    # the actual call order (optimizer.step → scheduler.step) is correct.
    ...
    for kspace, mask, target in loader:
        ...
        scaler.update()
        scheduler.step()   # OneCycleLR: one step per batch, not per epoch
        loss_val = loss.item()
        if math.isfinite(loss_val):   # GradScaler skips opt step on inf; skip accumulation too
            total += loss_val * kspace.size(0)
            n_finite += kspace.size(0)
    denom = n_finite if n_finite > 0 else len(loader.dataset)
    return total / denom
...
def main():
    ...
    print(f"  Spectral: {sum(p.numel() for p in model.spectral.parameters()):>9,} (K={cfg.n_spec_bases})")
    print(f"  HashFeat: {sum(p.numel() for p in model.hashfeat.parameters()):>9,}")
    print(f"  Cascade : {sum(p.numel() for p in model.cascade[0].parameters()):>9,} ({cfg.n_cascade} steps, shared weights)")
    print(f"  TOTAL   : {total_p:>9,} (~{total_p/1e6:.2f}M Params)\n")

    # Resume from checkpoint BEFORE torch.compile — compiled keys differ
    start_epoch = 1
    best_loss   = float("inf")
    ...
    # torch.compile after checkpoint load — avoids key mismatch on resume
    if cfg.device.type == "cuda" and hasattr(torch, "compile"):
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        t0 = time.time()
        tr = train_epoch(model, optimizer, scheduler, train_loader, criterion, cfg, scaler)
